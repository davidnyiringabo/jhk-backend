...
exports.getAllAppointments = async (req,res)=>{
    ...
    try{
        ...
        return res.status(500).send({message: "Error occurred while getting appointments!", status: 500});
    }
}
exports.createAppointment = async (req,res)=>{
    ...
    const id = uuidv4();
    console.log(id)
    const query = "INSERT INTO appointments (id,patientname, patientage, doctor_id, date, time, feepaid) VALUES ($1,$2,$3,$4,$5,$6,false);";
    try{
        const appointments = await client.query(query,[id, patientName, patientAge, doctorId, date, time]);
        return res.status(200).send({message: "Created appointment successfully!", status: 200});
    }
    catch(err){
        console.log("Error occurred while creating appointments!", err);
        return res.status(500).send({message: "Error occurred while creating appointments!", status: 500});
    }
}
